...
## 10. Plataforma de notificaciones event-driven (issue #37)

Documento de diseño completo: [`docs/notifications/ARCHITECTURE.md`](notifications/ARCHITECTURE.md). Resumen: los triggers Postgres en `bonds`/`transfers`/`reports` insertan filas en `outbox_events` **en la misma transacción** que el write de dominio (el codebase no tiene transacciones a nivel app, así que esto es la única forma atómica de capturar eventos). Un `OutboxDispatcher` por polling drena el outbox con orden por destinatario, retry+backoff+jitter, circuit breaker por canal, DLQ y `notification_dedup`; resuelve destinatarios, pasa por el routing engine puro (opt-outs, quiet hours con DST vía `luxon`, cadencia digest) y entrega por adaptadores de canal (in-app real; email/web-push con proveedores Noop hasta tener credenciales).

### Catálogo de eventos (outbox → templates → destinatarios)

Los triggers emiten `event_type` como `'<aggregate>.' || status` (literales en español del dominio). El `StaticTemplateEngine` usa el mismo string como `templateId` (`apps/api/src/notifications/templates/template-engine.ts`).

| `event_type` (trigger) | Template ID | Destinatarios |
|---|---|---|
| `bond.created` | `bond.created` | Dueño actual (`payload.currentOwner`) |
| `bond.congelado` | `bond.congelado` | Dueño actual |
| `bond.en_venta` | `bond.en_venta` | Dueño actual |
| `bond.aprobado` | `bond.aprobado` | Dueño actual |
| `bond.rechazado` | `bond.rechazado` | Dueño actual |
| `bond.activo` | `bond.activo` | Dueño actual |
| `transfer.requested` | `transfer.requested` | Ambas partes (`fromOwner`, `toOwner`) |
| `transfer.aceptada` | `transfer.aceptada` | Ambas partes |
| `transfer.rechazada` | `transfer.rechazada` | Ambas partes |
| `transfer.en_escrow` | `transfer.en_escrow` | Ambas partes |
| `transfer.pago_registrado` | `transfer.pago_registrado` | Ambas partes |
| `transfer.pago_validado` | `transfer.pago_validado` | Ambas partes |
| `transfer.liberada` | `transfer.liberada` | Ambas partes |
| `transfer.cancelada` | `transfer.cancelada` | Ambas partes |
| `report.enviado` | `report.enviado` | Miembros del partido (`party_id`) + todos los usuarios `tse` |
| `report.revisado` | `report.revisado` | Miembros del partido + TSE |
| `report.observado` | `report.observado` | Miembros del partido + TSE |
| `report.aprobado` | `report.aprobado` | Miembros del partido + TSE |

Resolución en `PostgresRecipientDirectory` (`apps/api/src/notifications/domain/postgres-recipient-directory.ts`): bonos → `[currentOwner]`; transferencias → `[fromOwner, toOwner]` deduplicados; reportes → `profiles` con `party_id = payload.partyId` ∪ `profiles.role = 'tse'`.

Templates auxiliares: `notification.digest` (compilador de resúmenes), `notification.generic` (fallback).

### Catálogo legacy: `NotificationType` + `NotificationsService.emit()`

El mecanismo **original** sigue activo en paralelo (transición deliberada, no bug): `packages/types/src/notification.ts` define `NotificationType` (`offer_received`, `offer_accepted`, `offer_rejected`, `counter_offer_received`, `payment_confirmed`, `bond_approved`, `bond_rejected`, `bond_request_received`, `report_submitted`, `report_observed`, `report_approved`, `report_resubmitted`) y `NotificationsService.emit(userId, type, payload)` inserta directo en `notifications` (fire-and-forget). Call sites actuales: `bonds.service.ts`, `transfers.service.ts`, `reports.service.ts`, `report-lifecycle.service.ts`. El pipeline outbox es el camino nuevo y general; los `emit()` directos extienden lo que ya funcionaba sin reescribirlo.

### Cómo conectar un proveedor real de canal

1. Implementar `EmailProvider` o `WebPushProvider` contra el SDK del vendor (`apps/api/src/notifications/channels/email-digest.channel.ts`, `web-push.channel.ts`).
2. En `notifications.module.ts`, factory `NOTIFICATION_CHANNELS`: construir `EmailChannel`/`WebPushChannel` con el proveedor real en lugar de `NoopEmailProvider`/`NoopWebPushProvider`. Nada más cambia — ese es el propósito de la interfaz.

### Cómo conectar observabilidad real

Implementar `MetricsRecorder` (`domain/observability.interface.ts`) y/o `Tracer` (`domain/tracing.interface.ts`) contra Prometheus, Datadog, OpenTelemetry, etc., y reemplazar el wiring de `METRICS_RECORDER` (hoy `InMemoryMetricsRecorder`) en `notifications.module.ts`. Mismo patrón swap-an-interface. Snapshot SLI in-process disponible hoy: `GET /notifications/admin/metrics` (solo `tse`/`admin`).

### Transporte realtime (gap conocido)

`WebSocketRealtimeTransport` (`transport/websocket-transport.ts`, paquete `ws`) existe pero **no está cableado** a un HTTP upgrade / gateway NestJS; producción usa `InMemoryRealtimeTransport` (process-local). Para activarlo: gateway o handler de upgrade que llame `registerSocket()` al conectar, luego cambiar el provider `REALTIME_TRANSPORT` en `notifications.module.ts`.

### Endpoints nuevos (además del list/read legacy)

```
GET    /notifications/inbox          (filtros + cursor)
GET    /notifications/grouped        (conteos por categoría)
PATCH  /notifications/bulk-read      ({ ids: string[] })
PATCH  /notifications/:id/archive
PATCH  /notifications/:id/unarchive
GET    /notifications/preferences
PATCH  /notifications/preferences/channels
PATCH  /notifications/preferences/quiet-hours
PATCH  /notifications/preferences/digest
GET    /notifications/admin/metrics    (tse/admin)
```

### Comprobación local sin credenciales

```bash
cd apps/api && npx jest notifications
```

(o `npx jest` completo). Toda la suite corre con fakes in-memory: sin `SUPABASE_*`, sin Postgres live, sin credenciales de email/push.
